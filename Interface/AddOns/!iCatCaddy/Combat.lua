local speed = .02 -- how fast the text appears
...
local a = CreateFrame ("Frame")
	...
	a:SetScript("OnEvent", function(self, event, _, subevent, _, _, sourceName, _, _, _, destName, _, _, _, _, _, spellID, spellName)
		if (UnitIsDead("player")) then return end
		if event == "PLAYER_REGEN_ENABLED" then allertrun("- 脱离战斗", 0.1, 1, 0.1) else allertrun("+ 进入战斗", 1, 0.1, 0.1) end
	end)