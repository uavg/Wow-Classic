...
--tag method: oUF_Simple:healthper
oUF.Tags.Methods["oUF_Simple:healthper"] = function(unit)
  if not UnitIsConnected(unit) then
    return "|cff999999Offline|r"
  end
  if(UnitIsDead(unit) or UnitIsGhost(unit)) then
  ...
  return L.F.NumberFormat(hpper).."%|ccccccccc"
end
--tag event: oUF_Simple:healthper
oUF.Tags.Events["oUF_Simple:healthper"] = "UNIT_HEALTH UNIT_MAXHEALTH UNIT_CONNECTION"
